...
# S-boxes (8 boxes, each with 4 rows and 16 columns)
S_BOX = [
    # S1
    [
...
# --- DES Core Functions ---
def des_crypt(block, subkeys, encrypt=True):
    ...
    for subkey in subkeys:
        ...
        for i in range(8):
            ...
            col = int(bits[1:5], 2)
            sbox_output.append(format(S_BOX[i][row][col], '04b'))
        
        sbox_result = ''.join(sbox_output)
        p_result = permute(sbox_result, P)
        
        new_right = format(int(left, 2) ^ int(p_result, 2), '032b')
        left, right = right, new_right
    
    combined = right + left
    ciphertext = permute(combined, FP)
    return bytes(int(ciphertext[i:i+8], 2) for i in range(0, 64, 8))

# --- Main Functions ---
def des_encrypt(plaintext, key):
    key = key.ljust(8)[:8].encode('utf-8')  # Explicit UTF-8
    subkeys = generate_subkeys(key)
    
    # Pad with UTF-8 aware padding
    plaintext_bytes = plaintext.encode('utf-8')
    plaintext_padded = pad_text(plaintext_bytes)
    
    ciphertext = bytearray()
    for i in range(0, len(plaintext_padded), 8):
        block = plaintext_padded[i:i+8]
        ciphertext += des_crypt(block, subkeys, True)
...
def des_decrypt(ciphertext, key):
    ...
    for i in range(0, len(ciphertext_bytes), 8):
        ...
        plaintext += des_crypt(block, subkeys, False)
    
    return unpad_text(plaintext).decode('utf-8')  # Decode as UTF-8

# --- CLI Handling ---
if __name__ == '__main__':
    if len(sys.argv) != 4:
        print("Usage: des.py [encode|decode] <text> <key>")
        sys.exit(1)
