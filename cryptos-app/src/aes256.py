...
def expand_key(key):
    key_size = 32  # AES-256 uses 32 bytes (256 bits) key
    expanded_key_size = 240  # AES-256 has 14 rounds + 1 initial round = 15 round keys of 16 bytes each = 240 bytes
    expanded_key = [0] * expanded_key_size
    current_size = 0
    rcon_iteration = 1
    
    # Copy the key to the first 32 bytes of expanded key
    ...
    while current_size < expanded_key_size:
        ...
        # Every 32 bytes (key_size), we apply the core function
        if current_size % key_size == 0:
            t = core(t, rcon_iteration)
            rcon_iteration += 1
        # In AES-256, we also apply SubBytes in the middle of each key expansion cycle
...
def aes_main(state, expanded_key):
    nbr_rounds = 14  # AES-256 has 14 rounds
...
def aes_encrypt_string(plaintext, key):
    if len(key) > 32:
        print("Warning: The key has been truncated to 32 bytes. This may reduce security. It is recommended to provide a 32-byte key.")
        key = key[:32]
    elif len(key) < 32:
        print("Warning: The key has been padded to 32 bytes. It is recommended to provide a 32-byte key.")
        key = key.ljust(32, '\0')
...
#     parser = argparse.ArgumentParser(description="AES-256 encryption and decryption implementation")  
...
#     parser.add_argument("--key", type=str, required=True, help="Encryption key (32 characters for AES-256)")  
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python aes256.py [encode|decode] [text] [key]", file=sys.stderr)
        sys.exit(1)
